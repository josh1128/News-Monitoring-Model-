...
export async function collectNews(p: CollectParams): Promise<CollectResult> {
  type Target = { entity: string; region: string; terms: string[]; ric: string | null };
  let targets: Target[] = [];

  if (p.mode === "sovereigns") {
    targets = Object.entries(SOVEREIGNS)
      .filter(([c]) => p.entityFilter.length === 0 || p.entityFilter.includes(c))
      .map(([country, terms]) => ({ entity: country, region: "Sovereign", terms, ric: null }));
  } else {
    ...
    for (const region of regions) {
      for (const b of BANKS_BY_REGION[region] ?? []) {
        ...
        targets.push({ entity: b.name, region, terms: [b.name, ...(b.terms ?? [])], ric: b.ric });
      }
    }
  }
  ...
  if (p.provider === "lseg") {
    if (!lsegEnabled()) {
      warnings.push("LSEG provider is off. Set LSEG_ENABLED=true + machine credentials on a PRIVATE deployment.");
    } else {
      const settled = await Promise.allSettled(
        targets.map((t) =>
          fetchLseg({ entity: t.entity, region: t.region, ric: t.ric, terms: t.terms, from: p.from, to: p.to, pageSize: p.limit })
        )
      );
      settled.forEach((r, i) => {
        if (r.status === "fulfilled") raw.push(...r.value);
        else warnings.push(`LSEG · ${targets[i].entity}: ${String((r as PromiseRejectedResult).reason?.message ?? "")}`);
      });
    }
  }
