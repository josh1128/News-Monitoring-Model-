...
## LSEG / Reuters (private only) ⚠️

Reuters/LSEG content is **licensed** — publishing it on a public URL is a
redistribution breach. This provider is therefore **off by default** and must
only be enabled on a **private/authenticated** deployment (Vercel Password
Protection or SSO).

To enable (private deployments only):
1. Protect the deployment (Vercel → Settings → Deployment Protection).
2. Set env vars:
   - `LSEG_ENABLED=true`
   - `LSEG_CLIENT_ID` (App Key), `LSEG_USERNAME` (Machine ID), `LSEG_PASSWORD`
   These must be **machine credentials** (Data Platform / RDP), not a desktop
   Workspace login.
3. Redeploy. Pick **LSEG / Reuters (private)** in the source dropdown.

Without both the flag and the credentials, the LSEG option returns a warning and
no articles — it can't be exposed by accident. Reuters articles have no public
link; the card shows a **Read full article** button that pulls the licensed text
in-app via `/api/story`. Response shapes vary by entitlement, so verify the
parser in `lib/lseg.ts` on your first live run.
